...
class T_merged_sim_log_transaction extends T_vsms_base_6_util {

    T_sim_log_transaction p_req = GC_NULL_OBJ_REF as T_sim_log_transaction
    T_sim_log_transaction p_resp = GC_NULL_OBJ_REF as T_sim_log_transaction

    @I_black_box("error")
    T_sim_log_transaction get_req() {
        return p_req
    }

    @I_black_box("error")
    void set_req(T_sim_log_transaction i_req) {
        p_req = i_req
    }

    @I_black_box("error")
    T_sim_log_transaction get_resp() {
        return p_resp
    }

    @I_black_box("error")
    void set_resp(T_sim_log_transaction i_resp) {
        p_resp = i_resp
    }
